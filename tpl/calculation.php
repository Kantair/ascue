...
$.fn.datetimepicker.defaults = {
  pickDate: true,            // enables the date picker
...
<?php
//опрос по расчетному каналу счетчика ВБМ для графика

$result = mssql_query("select (a.PAR1-b.PAR2) as VALUE0,timestamp = DATEDIFF(s, '19700101', a.DATA_DATE) from (SELECT VALUE0 as PAR1,DATA_DATE FROM DATA where ITEM='$item1' and OBJECT='$id_object' and PARNUMBER=12) a, (SELECT VALUE0 as PAR2,DATA_DATE,ITEMID FROM DATA where ITEM='$item2' and OBJECT='$id_object' and PARNUMBER=12) b where a.DATA_DATE=b.DATA_DATE and (a.DATA_DATE>='$date_ot_sql' and a.DATA_DATE<'$date_do_sql') order by a.DATA_DATE DESC, b.DATA_DATE DESC");
...
﻿<?php 
//опрос по расчетному каналу счетчика ВБМ
$result = mssql_query("select STR(a.PAR1-b.PAR2,8,2) as VALUE0,Convert(Varchar(50),a.DATA_DATE, 120) as DATA_DATE from (SELECT VALUE0 as PAR1,DATA_DATE FROM DATA where ITEM='$item1' and OBJECT='$id_object' and PARNUMBER=12) a, (SELECT VALUE0 as PAR2,DATA_DATE,ITEMID FROM DATA where ITEM='$item2' and OBJECT='$id_object' and PARNUMBER=12) b where a.DATA_DATE=b.DATA_DATE and (a.DATA_DATE>='$date_ot_sql' and a.DATA_DATE<'$date_do_sql') order by a.DATA_DATE, b.DATA_DATE");
...
$m=date('n');
$d=date('j');
$result = mssql_query("select SUM(summa)/2 as summa from (select (a.PAR1-b.PAR2) as summa, a.DATA_DATE from (SELECT VALUE0 as PAR1,DATA_DATE FROM DATA where ITEM='$item1' and OBJECT='$id_object' and PARNUMBER=12) a, (SELECT VALUE0 as PAR2,DATA_DATE FROM DATA where ITEM='$item2' and OBJECT='$id_object' and PARNUMBER=12) b where a.DATA_DATE=b.DATA_DATE ) c where Year(DATA_DATE)=$y and Month(DATA_DATE)=$m and Day(DATA_DATE)=$d");
$row = mssql_fetch_array($result);
echo "\t<tr border=1>\n";
echo "\t\t<td border=1>За текущий день</td>\n";
echo "\t\t<td border=1>" . $row["summa"] . "</td>\n";
echo "\t</tr>\n";
$result = mssql_query("select SUM(summa)/2 as summa from (select (a.PAR1-b.PAR2) as summa, a.DATA_DATE from (SELECT VALUE0 as PAR1,DATA_DATE FROM DATA where ITEM='$item1' and OBJECT='$id_object' and PARNUMBER=12) a, (SELECT VALUE0 as PAR2,DATA_DATE FROM DATA where ITEM='$item2' and OBJECT='$id_object' and PARNUMBER=12) b where a.DATA_DATE=b.DATA_DATE ) c where Year(DATA_DATE)=$y and Month(DATA_DATE)=$m and Day(DATA_DATE)=($d-1)");
$row = mssql_fetch_array($result);
echo "\t<tr border=1>\n";
...
setcookie("item1", $item1, time() + 60); 
setcookie("item2", $item2, time() + 60);
setcookie("id_object", $id_object, time() + 60); 
?>
<input type="submit" name="export" class="btn btn-primary" value="Вывести отчет" onclick="window.open('?q=export/exportcal')"><br> 
</form>
